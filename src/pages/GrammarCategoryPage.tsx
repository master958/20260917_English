...
export function GrammarCategoryPage() {
  ...
  if (category === null || !categoryId) {
    return <p className="text-gray-500">카테고리를 찾을 수 없습니다.</p>;
  }
  ...
  return (
    <div className="space-y-6">
      <Link to="/grammar" className="text-sm text-blue-600 hover:underline">
        ← 문법 목록으로
      </Link>
      <h1 className="text-2xl font-bold text-gray-900">{category.title}</h1>

      <section className="rounded-lg border border-gray-200 bg-white p-4">
        <h2 className="font-semibold text-gray-900">규칙 설명</h2>
        <p className="mt-2 text-sm text-gray-700">{category.ruleExplanation}</p>
        <h3 className="mt-4 font-semibold text-gray-900">예문</h3>
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-gray-900">퀴즈</h2>
        {result && (
